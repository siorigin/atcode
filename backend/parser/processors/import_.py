...
from parser.languages.lua import (
    extract_lua_assigned_name,
    extract_lua_pcall_second_identifier,
)
from parser.languages.rust import extract_rust_use_imports
...
class ImportProcessor:
    """Thread-safe processor for parsing and resolving import statements.

    All write operations to import_mapping are protected by a lock to enable
    parallel definition processing.
    """
    ...
    def __init__(
        ...
        self._lock = threading.Lock()  # Protects write operations to import_mapping

        # Dynamic import tracker for importlib.import_module() and getattr() patterns
        from .dynamic_import_tracker import DynamicImportTracker

        self.dynamic_import_tracker = DynamicImportTracker()
    ...
    def parse_imports(
        ...
        """Parse import statements and build import mapping for the module. Thread-safe.

        Note: Each file has a unique module_qn, so writes to import_mapping[module_qn]
        don't conflict between threads. We only need the lock for the initial dict assignment.
        """
        if language not in queries or not queries[language].get("imports"):
            # Still process dynamic imports even if no import query exists
            if language == "python":
                # Initialize module's import mapping (thread-safe)
                with self._lock:
                    if module_qn not in self.import_mapping:
                        self.import_mapping[module_qn] = {}
                self._process_dynamic_imports(root_node, module_qn, language)
            return

        lang_config = queries[language]["config"]
        imports_query = queries[language]["imports"]

        # Initialize module's import mapping (thread-safe) - only this needs the lock
        with self._lock:
            self.import_mapping[module_qn] = {}

        # Process imports WITHOUT holding the lock - each module_qn is unique per thread
        try:
            ...
            # Process dynamic imports (Python only for now)
            if language == "python":
                self._process_dynamic_imports(root_node, module_qn, language)

            logger.debug(
                f"Parsed {len(self.import_mapping.get(module_qn, {}))} imports in {module_qn}"
            )

            # Create IMPORTS relationships for each parsed import
            # Skip stdlib imports to reduce graph noise
            if self.ingestor and module_qn in self.import_mapping:
                for local_name, full_name in self.import_mapping[module_qn].items():
                    ...
                    # This ensures File -> File relationships, not File -> Class/Function
                    module_path = self._extract_module_path(full_name, language)

                    # Skip stdlib imports - they create too many relationships
                    # and don't provide useful information for code understanding
                    if StdlibChecker.is_stdlib(module_path, language):
                        logger.debug(
                            f"  Skipping stdlib IMPORTS: {module_qn} -> {module_path}"
                        )
                        continue
                    ...
                    self.ingestor.ensure_relationship_batch(
                        ...
                        "IMPORTS",
                        ("File", "qualified_name", module_path),
                    )
                    logger.debug(
    ...
    def _process_dynamic_imports(
        ...
        """Process dynamic imports (importlib.import_module + getattr patterns).
        ...
        """
        dynamic_mappings = self.dynamic_import_tracker.process_module(
            root_node, module_qn, language
        )

        # Merge dynamic imports into import_mapping
        if module_qn not in self.import_mapping:
            self.import_mapping[module_qn] = {}

        for var_name, qualified_name in dynamic_mappings.items():
            self.import_mapping[module_qn][var_name] = qualified_name
            logger.debug(
                f"Added dynamic import to mapping: {var_name} -> {qualified_name}"
            )

    def get_discovered_externals(self) -> set[str]:
        """Return all external modules discovered via dynamic imports."""
        return self.dynamic_import_tracker.get_discovered_externals()
    ...
    def _lua_is_stdlib_call(self, call_node: Node) -> bool:
        ...
        from parser.languages.lua import safe_decode_text
