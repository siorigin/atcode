...
from parser.languages.java_type_inference import JavaTypeInferenceEngine
from parser.languages.js_type_inference import JsTypeInferenceEngine
from parser.languages.lua_type_inference import LuaTypeInferenceEngine
from parser.languages.python import resolve_class_name
from parser.utils import JS_TYPESCRIPT_LANGUAGES
from tree_sitter import Node, QueryCursor

from .import_ import ImportProcessor

if TYPE_CHECKING:
    from parser.factory import ASTCacheProtocol
...
class TypeInferenceEngine:
    ...
    def __init__(
        ...
        # Cache for method qualified name to module qualified name mapping
        # This avoids iterating through all files in ast_cache for every method lookup
        self._method_qn_to_module_qn_cache: dict[str, str] = {}

        # Track attributes currently being inferred to prevent infinite recursion
        # This happens when _infer_attribute_type -> _analyze_self_assignments ->
        # _infer_type_from_expression -> _infer_method_call_return_type ->
        # _resolve_method_qualified_name -> _infer_attribute_type (loop)
        self._inferring_attributes: set[tuple[str, str]] = set()
    ...
    def build_local_variable_type_map(
        ...
        if language == "python":
            ...
            pass
        elif language in JS_TYPESCRIPT_LANGUAGES:
    ...
    def _infer_attribute_type(self, attribute_name: str, module_qn: str) -> str | None:
        ...
        # Check for recursion: if we're already inferring this attribute, skip to avoid infinite loop
        cache_key = (attribute_name, module_qn)
        if cache_key in self._inferring_attributes:
            logger.debug(
                f"Skipping recursive inference for attribute {attribute_name} in {module_qn}"
            )
            return None

        # Mark this attribute as being inferred
        self._inferring_attributes.add(cache_key)
        ...
        try:
            ...
            for file_path, (root_node, language) in self.ast_cache.items():
                ...
                try:
                    ...
                    # Skip files that are not relative to repo_path
                    logger.debug(
                        f"Skipping file not relative to repo_path: {file_path}"
                    )
                    continue
            ...
            self._inferring_attributes.discard(cache_key)
    ...
    def _find_method_ast_node(self, method_qn: str) -> Node | None:
        ...
        # Check cache first
        if method_qn in self._method_qn_to_module_qn_cache:
            cached_module_qn = self._method_qn_to_module_qn_cache[method_qn]
            if cached_module_qn in self.module_qn_to_file_path:
                file_path = self.module_qn_to_file_path[cached_module_qn]
                ...
                del self._method_qn_to_module_qn_cache[method_qn]

        # Fast path: Try direct module_qn lookup first
        if expected_module in self.module_qn_to_file_path:
            ...
            if file_path in self.ast_cache:
                ...
                if result:
                    ...
                    self._method_qn_to_module_qn_cache[method_qn] = expected_module
                    return result

        # Slow path: Fall back to iterating through ast_cache (only if fast path fails)
        # This should rarely happen if module_qn_to_file_path is properly maintained
        for file_path, (root_node, language) in self.ast_cache.items():
            ...
            try:
                ...
                # Skip files that are not relative to repo_path
                logger.debug(f"Skipping file not relative to repo_path: {file_path}")
                continue
            ...
            if parts and parts[0] == project_name:
                parts = parts[1:]  # Skip duplicate project name
            file_module_qn = ".".join([project_name] + parts)

            if file_path.name == "__init__.py":
                parent_parts = list(relative_path.parent.parts)
                if parent_parts and parent_parts[0] == project_name:
                    parent_parts = parent_parts[1:]
                file_module_qn = ".".join([project_name] + parent_parts)
            ...
            if file_module_qn == expected_module:
                ...
                if result:
                    # Cache the successful lookup
                    self._method_qn_to_module_qn_cache[method_qn] = file_module_qn
                    return result
