...
from core.language_config import LANGUAGE_CONFIGS
from loguru import logger
from tree_sitter import Language, Parser, Query

logger.remove()
log_level = os.getenv("LOG_LEVEL", os.getenv("LOGURU_LEVEL", "INFO"))
...
def load_parsers(
    ...
    tuple[dict[str, Parser], dict[str, Any]]
    | tuple[dict[str, Parser], dict[str, Any], dict[str, Language]]
):
    """Loads all available Tree-sitter parsers and compiles their queries.

    Args:
        return_languages: If True, also return the Language objects for parser pool creation

    Returns:
        If return_languages is False: (parsers, queries)
        If return_languages is True: (parsers, queries, languages)
    """
    ...
    for lang_name, lang_config in configs.items():
        if lang_lib := LANGUAGE_LIBRARIES.get(lang_name):
            try:
                ...
                languages[lang_name] = language  # Store for parser pool
                ...
                if lang_name in ("javascript", "typescript"):
                    ...
                    try:
                        js_variables_patterns = """
                        (variable_declaration) @variable
                        (lexical_declaration) @variable
                        """
                        variables_query = Query(language, js_variables_patterns)
                    except Exception as e:
                        logger.debug(
                            f"Failed to create variables query for {lang_name}: {e}"
                        )

                elif lang_name == "python":
                    # Python module-level and class-level variable assignments
                    try:
                        python_variables_patterns = """
                        (expression_statement (assignment) @variable)
                        """
                        variables_query = Query(language, python_variables_patterns)
                    except Exception as e:
                        logger.debug(
                            f"Failed to create variables query for {lang_name}: {e}"
                        )
                    ...
                    # Rust static/const declarations
                    try:
                        rust_variables_patterns = """
                        (static_item) @variable
                        (const_item) @variable
                        """
                        variables_query = Query(language, rust_variables_patterns)
                    except Exception as e:
                        logger.debug(
                            f"Failed to create variables query for {lang_name}: {e}"
                        )
                    ...
                    try:
                        go_variables_patterns = """
                        (var_declaration) @variable
                        (const_declaration) @variable
                        """
                        variables_query = Query(language, go_variables_patterns)
                    except Exception as e:
                        logger.debug(
                            f"Failed to create variables query for {lang_name}: {e}"
                        )
    ...
    if return_languages:
        return parsers, queries, languages
