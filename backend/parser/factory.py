...
from .processors.call import CallProcessor
from .processors.definition import DefinitionProcessor
from .processors.import_ import ImportProcessor
from .processors.structure import StructureProcessor
from .processors.type_inference import TypeInferenceEngine
...
class ProcessorFactory:
    ...
    def __init__(
        ...
        self.gitignore_parser = None  # Set externally before identify_structure()
    ...
    def structure_processor(self) -> StructureProcessor:
        ...
        if self._structure_processor is None:
            self._structure_processor = StructureProcessor(
                ...
                gitignore_parser=self.gitignore_parser,
            )
    ...
    def call_processor(self) -> CallProcessor:
        ...
        if self._call_processor is None:
            self._call_processor = CallProcessor(
                ...
                class_inheritance=self.definition_processor.class_inheritance,
                callers_index=getattr(self, "_callers_index", None),
            )
        return self._call_processor

    def set_callers_index(self, callers_index) -> None:
        """Set the callers index for call tracking.

        Args:
            callers_index: CallersIndex instance for reverse call tracking
        """
        self._callers_index = callers_index
        # If call processor already exists, update it
        if self._call_processor is not None:
            self._call_processor.set_callers_index(callers_index)
