...
def build_cpp_qualified_name(node: Node, module_qn: str, name: str) -> str:
    ...
    This function constructs qualified names for C++ functions, classes, and other
    entities while avoiding namespace duplication. When a namespace name appears
    both in the file path (module_qn) and in the code's namespace definition, we
    filter it out to prevent redundant qualified names.

    Args:
        node: The AST node representing the C++ entity
        module_qn: The module qualified name (e.g., "vllm.csrc.cache_kernels")
        name: The entity name (e.g., "concat_and_cache_mla_kernel")

    Returns:
        The qualified name for the entity

    Examples:
        Case 1 - Namespace matches project name (common in vLLM):
            module_qn = "vllm.csrc.cache_kernels"
            namespace = "vllm"
            name = "concat_and_cache_mla_kernel"
            Result: "vllm.csrc.cache_kernels.concat_and_cache_mla_kernel"
            (NOT "vllm.csrc.cache_kernels.vllm.concat_and_cache_mla_kernel")

        Case 2 - Nested namespaces with partial overlap:
            module_qn = "project.utils.helpers"
            namespace = "utils::helpers::internal"
            name = "process"
            Result: "project.utils.helpers.internal.process"
            (Filters out "utils" and "helpers" which are already in module_qn)

        Case 3 - No namespace duplication:
            module_qn = "project.core.engine"
            namespace = "cuda"
            name = "kernel_launch"
            Result: "project.core.engine.cuda.kernel_launch"
    """
    ...
    if is_module_file:
        ...
        # Filter out namespaces that are already in module_qn to prevent duplication
        # Example: if module_qn is "vllm.csrc.cache_kernels" and namespace is "vllm",
        # we skip "vllm" to avoid "vllm.csrc.cache_kernels.vllm.func"
        filtered_parts = []
        ...
        if filtered_parts:
            return f"{module_qn}.{'.'.join(filtered_parts)}.{name}"
        else:
